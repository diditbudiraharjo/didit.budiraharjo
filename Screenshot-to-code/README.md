...
> Vendored from [emilwallner/Screenshot-to-code](https://github.com/emilwallner/Screenshot-to-code) (MIT / Apache-2.0, see `LICENSE`).
>
> ```bash
...
> ```
>
> Verified: the dependencies in `requirements.txt` install cleanly (TensorFlow 2.21, Keras 3.15) and import without error on Python 3.11. The notebooks themselves target the legacy Keras 2 API (e.g. `keras.preprocessing.text`, `keras.layers.core`), so running the training cells as-is on Keras 3 will need those calls updated to their modern equivalents.
